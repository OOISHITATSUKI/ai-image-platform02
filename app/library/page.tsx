...
type PageTab = 'generation' | 'billing';
    ...
    if (!isAuthenticated && typeof window !== 'undefined' && !getToken()) {
        return (
            <div style={styles.emptyContainer}>
                ...
                <h2 style={styles.emptyTitle}>{t('history.loginRequired')}</h2>
                <p style={styles.emptyDesc}>{t('history.loginRequiredDesc')}</p>
                <Link href="/login" style={styles.ctaButton}>{t('auth.login')}</Link>
            </div>
        );
    }
        ...
        <div style={styles.page}>
            ...
            <div style={styles.header}>
                <div style={styles.headerLeft}>
                    <button onClick={() => router.push('/editor')} style={styles.backBtn}>← {t('history.back')}</button>
                    <h1 style={styles.title}>{t('history.title')}</h1>
                    <span style={styles.countBadge}>{total} {t('history.items')}</span>
                </div>
            </div>

            {/* ── Page Tabs (Generation / Billing) ── */}
            <div style={styles.pageTabBar}>
                <button
                    style={{ ...styles.pageTab, ...styles.pageTabActive }}
                >
                    {t('history.generationTab')}
                </button>
                <button
                    onClick={() => router.push('/history/billing')}
                    style={styles.pageTab}
                >
                    {t('history.billingTab')}
                </button>
            </div>

            {/* ── Expiry Notice Banner ── */}
            <div style={styles.expiryBanner}>
                {t('history.expiryNotice')}
            </div>
            ...
            {loading && generations.length === 0 ? (
                <div style={styles.emptyContainer}>
                    ...
                    <p style={styles.emptyDesc}>{t('history.loading')}</p>
                </div>
            ) : error ? (
                <div style={styles.emptyContainer}>
                    <div style={{ fontSize: '3rem', marginBottom: 16 }}>⚠️</div>
                    <p style={styles.emptyDesc}>{error}</p>
                    <button onClick={() => fetchGenerations(0)} style={styles.ctaButton}>{t('history.retry')}</button>
                </div>
            ) : filtered.length === 0 ? (
                <div style={styles.emptyContainer}>
                    ...
                    <div style={styles.lightboxContent} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={styles.lightboxInfo}>
                            ...
                            <div style={styles.lightboxPrompt}>
                                <div style={styles.lightboxLabel}>{t('history.prompt')}</div>
                                <div style={styles.lightboxPromptText}>{lightbox.prompt}</div>
                            </div>

                            <div style={styles.lightboxDetailRow}>
                                <span style={styles.lightboxLabel}>{t('history.model')}</span>
                            ...
                            {Boolean(lightbox.params?.width) && Boolean(lightbox.params?.height) && (
                                <div style={styles.lightboxDetailRow}>
                                    <span style={styles.lightboxLabel}>{t('history.size')}</span>
                            ...
                            <div style={styles.lightboxDetailRow}>
                                <span style={styles.lightboxLabel}>{t('history.credits')}</span>
                            ...
                            <div style={styles.lightboxActions}>
                                ...
                                >
                                    {favoriteIds.has(lightbox.id) ? `❤️ ${t('history.favorited')}` : `🤍 ${t('actions.favorite')}`}
                                </button>
                                <button
                                    onClick={() => handleDownload(lightbox)}
                                    style={styles.lightboxActionBtn}
                                >
                                    ⬇️ {t('actions.download')}
                                </button>
                                <button
