    ...
    try {
        ...
        if (isImg2ImgValue) {
            ...
            if ((inpaintMode && maskBase64) || (isPureImg2Img && editAnalysis.region !== 'full')) {
                ...
                // Repose: resize main image to avoid API size limits
                const repBuf = Buffer.from(finalImageBase64, 'base64');
                const repResized = await sharp(repBuf).resize(768, 768, { fit: 'inside', withoutEnlargement: true }).jpeg({ quality: 85 }).toBuffer();
                novitaRequest.image_base64 = repResized.toString('base64');
                console.log('Repose: resized main image to 768px');
            } else {
